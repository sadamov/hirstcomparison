...
I tried to be less strict with removing data for the hourly and daily averages, but the resulting time series show some really large differences between the traps. It is not fair to compare the traps during calibration events and as discussed with Benoit Crouzy, the calibration can sometimes affect the measurements that come directly after. Hence it was suggested to remove the full calibration days. This will be consistent with the preparation for 3 and 6 and 12 hour averages. 
...
for (i in 1:ncol(traps_pairs)){
  ...
  geom_label(label = c(paste(ci[i], "\n", ci[ncol(traps_pairs) + i], "\n", ci[ncol(traps_pairs) * 2 + i])), aes(x = max(data_corr[traps_pairs[1, i]], na.rm = TRUE) + 1, y = max(data_corr[traps_pairs[1, i]], na.rm = TRUE) + 1)) +
...
gg_hist24 <- data_altman %>% 
  filter(`diff(poleno2 - poleno4)` > quantile(abs(data_altman$`diff(poleno2 - poleno4)`), 0.75, na.rm = TRUE)) %>% 
  ggplot()+
...
gg_hist25 <- data_altman %>% 
  filter(`diff(poleno2 - poleno5)` > quantile(abs(data_altman$`diff(poleno2 - poleno4)`), 0.75, na.rm = TRUE)) %>% 
  ggplot()+
  geom_histogram(aes(x = timestamp), bins = 50) +
...
conc_groups_scaled <- quantile(data_anova %>% filter(trap != "hirst") %>% pull(total), seq(0, 1, 1/6), na.rm = TRUE)
conc_groups_names <- paste0("Group", round(seq(0, 1, 1/6)[1:6], 2), "_", round(seq(0, 1, 1/6)[2:7], 2))

data_errors_comb_conc <- data_errors_raw %>% 
  mutate(conc = case_when(
    mean >= conc_groups_scaled[1] & mean < conc_groups_scaled[2] ~ conc_groups_names[1],
    mean >= conc_groups_scaled[2] & mean < conc_groups_scaled[3] ~ conc_groups_names[2],
    mean >= conc_groups_scaled[3] & mean < conc_groups_scaled[4] ~ conc_groups_names[3],
    mean >= conc_groups_scaled[4] & mean < conc_groups_scaled[5] ~ conc_groups_names[4],
    mean >= conc_groups_scaled[5] & mean < conc_groups_scaled[6] ~ conc_groups_names[5],
    mean >= conc_groups_scaled[6] ~ conc_groups_names[6]
  )) %>% 
...
names(labels_y) <- conc_groups_names
names(labels_y_hist) <- conc_groups_names

for (j in conc_groups_names){
