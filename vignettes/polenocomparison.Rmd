...
plot_comb(resolution = resolution, traps = c("poleno2", "poleno4", "poleno5", "hirst"), rm_zeros = TRUE)
...
data_corr <- data_transformed %>% # For the robust methods (spearman, kendall) it doesn't matter whether the transformed data is used or the original
  ...
  setNames(c("timestamp", c("hirst", paste0("poleno", c(2, 4, 5)))))
...
ci <- map(corr_matrix, ~.x %>% 
  ...
  bind_rows() %>% 
  round(2) %>% 
  mutate(method = rep(methods, each = 6)) %>%  
