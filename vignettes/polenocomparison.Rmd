...
resolution <- "3hour" # What temporal resolution should be plotted c("daily", "12hour", "6hour", "3hour", "hourly")
...
data_poleno <- map2(data_raw_poleno, c(2, 4, 5), ~.x %>% 
  ...
  filter(timestamp > ymd_hms("2020-02-19 00:00:00"),
         timestamp <= ymd_hms("2020-05-03 23:00:00")))
...
missing_hirst <- data_hirst %>% filter(is.na(total)) %>% pull(timestamp) # The NAs are actual NAs here, during timeslots when the pollen rubber bands were exchanged.

```

For the hourly values we rescale the measurements to a range from 0 to 100 fir hirst and poleno. The daily values and then based on those hourly averages. For the 3, 6 and 12 hour averages we will scale the resulting pollen counts for the poleno device to a scale from 0 to 100 as well.
...
data_hirst_scaled <- scales::rescale(data_hirst$total, c(0, 100))
...
We would expect to have a total of `r data_poleno_red %>% bind_rows %>% pull(date) %>% unique %>% length` 56 days with Poleno measurements that are not affected by calibration.
  ...
  for (j in 2:3){
    list_timeseries[[j]] <- x[[1]] %>% 
      ...
      mutate(date = date(timestamp),
             ...
             total = if_else(is.na(total), 0L, total))
  }
  list_timeseries
}
...
data_hours3_comb <- data_hours3_pol %>% 
  bind_rows() %>% 
  mutate(total = scales::rescale(.$total, c(0, 100)))
data_hours6_comb <- data_hours6_pol %>% 
  bind_rows() %>% 
  mutate(total = scales::rescale(.$total, c(0, 100)))
data_hours12_comb <- data_hours12_pol %>% 
  bind_rows() %>% 
  mutate(total = scales::rescale(.$total, c(0, 100)))
...
data_aggs_comb <- list(data_hourly_comb, data_daily_comb)
number_of_measurements_comb <- map(data_aggs_comb, ~.x %>% 
  ...
  mutate_at(vars(total), ~(as.integer(as.integer(.) != 0))) %>% 
  ungroup %>% 
  group_by(date, hour) %>%
  summarise_at(vars(total), ~sum(., na.rm = TRUE)) %>% 
  ungroup)

timesteps_comb <- map(data_aggs_comb, ~ (.x %>% nrow()) / length(traps))
...
number_traps_comb <- map2(number_of_measurements_comb, timesteps_comb, ~ bind_rows(.x %>% 
  ...
  rename("Hourly-Values" = total, "Daily-Values" = total1)
...
for (i in 1:ncol(traps_pairs)) {
  gg_ab[[i]] <- data_altman %>% 
    ...
    coord_cartesian(xlim = c(0, 5), ylim = c(-sd_diff[i] * 5, sd_diff[i] * 5)) +  
...
gg_hist <- data_altman %>% 
  filter(`diff(poleno2 - poleno5)` < -1) %>% 
  ggplot()+
  geom_histogram(aes(x = timestamp), bins = 50) 

gg_hist %>%  
  annotate_figure(bottom = text_grob("The largest differences occur during the first two weeks of March. According to the measurements at MCH \n  the species blooming during that period are Cupressus, Eibe, Erle, Esche, Pappel, Weide, Ulme, Hasel", color = swatch()[1], face = "italic", size = 10), top = "Main Differences Between Poleno 2 and Poleno 5")

```

```{r}
 data_altman %>% 
  filter(abs(`diff(poleno2 - poleno5)`) > 1) %>% 
  ggplot()+
  geom_histogram(aes(x = timestamp), bins = 50) 
```
...
errors <- data_corr %>% 
  mutate(exclude = rowSums(is.na(data_corr)) < length(traps), # At least one trap should have measured something otherise we are not calculating the means
    mean = if_else(exclude,
                   rowSums(.[2:length(traps) + 1], na.rm = TRUE) / ncol(traps_pairs), 
                   NA_real_)) %>% 
  na.omit() %>% 
  mutate_at(vars(all_of(traps)), ~(. - mean)) %>% 
  pivot_longer(cols = 2:(1 + length(traps)), values_to = "error", names_to = "trap") %>% 
