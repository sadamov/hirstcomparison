...
This analysis could look as complete as the Hirst one at some point, once the classifier is fully functional. Until then it is suggested to also consider the Hirstcomparison vignette for the latest methods and plots.
...
species <- "total" # c("total", "alnus", "corylus", "taxus", "ulmus", "cupressus" "betula",  "populus", "poaceae", "fraxinus"  "platanus"  "quercus", "fagus", "pinaceae", "carpinus")
...
files_data <- list.files(path_data, pattern = "second")
...
data_poleno <- map2(data_raw_poleno, c(2, 4, 5), ~.x %>%
  select(prediction_max_value, prediction_label, timestamp) %>%
  mutate(timestamp = as_datetime(timestamp),
         prediction_label = as_factor(prediction_label),
         date = date(timestamp),
         hour = hour(timestamp) + 1,
         trap = paste0("poleno", .y)) %>%
  filter(timestamp > ymd_hms("2020-03-03 00:00:00"),
         timestamp <= ymd_hms("2020-05-08 00:00:00")))

if (species != "total"){
  data_poleno <- map(data_poleno, ~.x %>% 
                             filter(prediction_label == species
                                    , prediction_max_value >= 0.1 # This is a simple approximation as Payerne (Gian) was still working on the classifier optimization.
  ))
}
...
data_hirst_raw <- data.table::fread(paste0(path_data, file_hirst)) %>% 
  ...
  filter(timestamp > ymd_hms("2020-03-03 00:00:00"), # That's when the Hirst traps started measuring this season (First Full Day)
         timestamp <= ymd_hms("2020-05-08 00:00:00")) %>% 
...
data_hirst_raw %>%
  ...
  Amelia::missmap(
    ...
    y.at = 1,
    col = swatch()[c(4, 2)],
    main = "Missingness Map for Hirst Trap from DWH"
  )
...
data_hirst_prep <- data_hirst_raw %>%
  ...
  rename(
    ...
    quercus = kaquerh0,
    fagus = kafaguh0,
    pinaceae = kapinuh0, # Pinus is probably a sub-selecion of pinaceae only (check with Payerne)
    carpinus = kacarph0
  ) %>%
  {if (species != "total") mutate(., total = !!sym(species)) else .} %>% 
  select(timestamp, date, hour, trap, total)



missing_hirst <-
  data_hirst_prep %>% filter(is.na(total)) %>% pull(timestamp) # The NAs are actual NAs here, during timeslots when the pollen rubber bands were exchanged.
...
# Those times have to be filtered for at least that specific trap. But, as we will compare traps with each other, it's safest to remove those times from all traps. As we are comparing with hourly averages from Hirst we should probably remove the full hours during which calibration was carried out. Our time series expands from the 15.2.2020 until the 3.5.2020. Removing the full day was also tested and considered to be too strict.
...
number_traps_comb <- map2(number_of_measurements_comb, timesteps_comb, ~ bind_rows(.x %>% 
  ...
  rename("Hourly-Values" = total...1,
         "3Hour-Values" = total...2,
         "6Hour-Values" = total...3,
         "12Hour-Values" = total...4,
         "Daily-Values" = total...5)

table_traps <- number_traps_comb %>% 
  bind_rows() %>% 
  kable() %>% 
...
# kableExtra::save_kable(table_traps, file = "table_traps.doc")
...
```{r}
data_aggs_pol <- list(data_daily_comb %>% filter(trap != "hirst"), 
                      data_hours12_pol %>% bind_rows, 
                      data_hours6_pol %>% bind_rows, 
                      data_hours3_pol %>% bind_rows, 
                      data_hourly_comb %>% filter(trap != "hirst")) 

map(data_aggs_pol, ~.x %>% 
    mutate(sd = sd(total, na.rm = TRUE),
           se = sd / 3,
           mean = mean(total, na.rm = TRUE),
           cv = sd/mean) %>% 
    filter(timestamp == as.Date("2020-03-05"),
           trap == "poleno2") %>% 
    select(sd, mean, se, cv)) %>% 
  bind_rows %>% 
  mutate_if(is.numeric, ~round(., 2)) %>% 
  mutate(Average = c("Daily", "12-Hours", "6-Hours", "3-Hours", "Hourly")) %>% 
...
data_anova %>%
  ...
  geom_line() +
  ggtitle(paste(tools::toTitleCase(resolution), tools::toTitleCase(species), "Pollen Counts")) +
  labs(x = "Date")
...
Ein paar Kommentare (auf Deutsch) die im Meeting mit MDS besprochen wurden und helfen die Time Series zu erklären:
Für poleno hat man sehr hohes sampling, deshalb haben polenos nicht mehr gemessen (der max threshold wurde erreicht an gewissen tagen). Am 16. april wurde threshold erreicht, poleno 5 vielleicht früher wege feinstaub
Am 6.7.8. april gab es wasser im gerät - problem mit poaceae. Bei gräsern wurden auf poleno2 tropfen gemessen.
Feinstaub in städten wird mit brennessel verwechselt.
...
data_corr <- data_transformed %>% # For the robust methods (spearman, kendall) it doesn't matter whether the transformed data is used or the original
  select(total, trap, timestamp) %>% 
  pivot_wider(names_from = trap, values_from = total, timestamp) %>% 
...
for (i in 1:ncol(traps_pairs)){
  ...
  geom_label(label = c(paste(ci[i], "\n", ci[ncol(traps_pairs) + i], "\n", ci[ncol(traps_pairs) * 2 + i])), aes(x = max(data_corr[traps_pairs[1, i]], na.rm = TRUE) - 1.5, y = 1)) +
  coord_cartesian(ylim = c(0, max(data_corr[traps_pairs[1, i]], na.rm = TRUE) + 2), xlim = c(0, max(data_corr[traps_pairs[1, i]], na.rm = TRUE) + 2))
...
ggarrange(plotlist = gg_corr) %>%
  annotate_figure(top = title, bottom = text_grob("Pairwise correlation of traps; grey line shows the Loess smother; the red line shows a theoretical perfect correlation of 1. \n In the text box one can see the 95% confidence intervals of the r-values (adjusted for multiple comparison) \n as obtained by Pearson and two robust methods.", color = swatch()[1], face = "italic", size = 12))
```
...
ggarrange(plotlist = gg_ab) %>%
  annotate_figure( top = title, bottom = text_grob("Pairwise comparison of means and differences between traps; grey line shows the Loess smother; \n the red line shows a theoretical perfect agreement between two traps of zero. \n The dashed red line represents a band with width ±1.96 * sd of the differences, where we expect the points to lie within.", color = swatch()[1], face = "italic", size = 12))
  

```

We observe that the largest differences occur at low and very high concentrations. Usually when poleno 2 is involved and hirst repsectively. In the following we are identifying the days on which the largest differences are occuring.
...
```{r}
fit_anova <- aov(total ~ trap, data = data_transformed, contrasts = c("contr.sum", "contr.poly"))
summary(fit_anova)
...
```{r}
fit_anova_raw <- aov(total ~ trap, data = data_anova, contrasts = c("contr.sum", "contr.poly"))
...
```{r}
fit_lmer <- lmer(total ~ (1 | trap), data = data_transformed)
...
The Wilcoxon signed-rank test assumes that the data are distributed symmetrically around the median. In other words, there should be roughly the same number of values above and below the median. This can be checked by visual inspection using histogram or density estimators. The kruskal wallis test assumes that the time series originate from the same underlying distribution.
...
data_transformed %>% 
  ...
  geom_vline(xintercept = median(data_transformed %>% pull(total), na.rm = TRUE), col = swatch()[4]) +
...
```{r}
kruskal.test(total ~ trap, data = data_transformed)
```
...
dunn.test::dunn.test(data_transformed %>% pull(total),
                     data_transformed$trap,
                     method = "holm") # Try different FEWR /FDR adjustments here

conover.test::conover.test(data_transformed %>% pull(total),
                     data_transformed$trap,
                     method = "holm")
...
# 
# pairwise.wilcox.test(data_anova %>% pull(total), 
#                      data_anova$trap,
#                      paired = FALSE,
#                      p.adjust.method = "fdr",
#                      conf.int = TRUE)
...
```{r eval = FALSE}
fit_rlmer <- rlmer(total ~ (1 | trap), data = data_transformed)
summary(fit_rlmer)
...
## Histograms of Concentration Groups

```{r fig.height=7, fig.width=10}
gg_hist_conc_pol <- list()

data_errors_pol <- data_hourly_comb %>%
  filter(trap != "hirst") %>% 
  group_by(timestamp) %>% 
  mutate(mean_conc = mean(total)) %>% 
  mutate(conc = case_when(
    mean_conc >= 0 & mean_conc < 10 ~ "Group00_10",
    mean_conc >= 10 & mean_conc < 20 ~ "Group10_20",
    mean_conc >= 20 & mean_conc < 50 ~ "Group20_50",
    mean_conc >= 50 & mean_conc < 100 ~ "Group50_100",
    mean_conc >= 100 & mean_conc < 300 ~ "Group100_300",
    mean_conc >= 300 ~ "Group300"),
    type = "total") %>% 
  filter(!is.na(conc)) %>% # We only want timestamps where all three traps measured
  select(timestamp, trap, total, conc, type) %>% 
  pivot_wider(names_from = conc, values_from = total) %>% 
  ungroup

data_hourly_comb %>% summary

conc_y_label <- c(1.3, 1.4, 1.3, 1.6, 3, 3)
...
for (j in c("Group00_10", "Group10_20", "Group20_50", "Group50_100", "Group100_300", "Group300")){
  gg_hist_conc_pol[[j]] <- plot_hist_dt(data_errors_pol, j, n_test = 1, samples = 1, plots = TRUE, tdist = FALSE, y_label = conc_y_label[j])
}

ggarrange(plotlist = gg_hist_conc_pol, nrow = 3, ncol = 2) %>% 
  annotate_figure(top = paste("Ratios of Individual Hourly Measurements of the Three Traps vs Average Measurements for Six Pollen Concentration Groups"), bottom = text_grob("We are looking at measurement ratios fitted with a density kernel estimator (red).", color = swatch()[1], face = "italic", size = 10))

```


A second chapter of the analysis should further elaborate on the relative differences between the traps.
...
This analysis is quite experimental and if continued should be re-evaluated. For example the imputation of missing data due to calibration events should not be zeros, but rather some sort of moving average.

```{r fig.width = 10, fig.height = 10, eval = FALSE}

if (!exists("data_raw_poleno")){
  load(file = "data_raw_poleno.Rdata")
...
if (!exists("ts_autocorr_tb")){
  ...
  while (start_window < ymd_hms("2020-05-08 00:00:00")){
...
ts_autocorr_tb <- ts_autocorr_tb %>% 
  mutate(count_window = log(count_window + 1)) # This is for 5-Minutes Data
...
ts_autocorr_tb <- data_hourly_comb %>% 
  filter(trap == "poleno2") %>% 
  select(timestamp, total) %>% 
  mutate(total = if_else(is.na(total), 0, total),
         total = log(total + 1)) %>% 
  rename(end_window = timestamp, count_window = total) # This is for hourly data

ts_autocorr <- as_tsibble(ts_autocorr_tb, regular = TRUE, index = end_window)
...
ts_autocorr_tb %>%
  ggplot(aes(x = end_window, y = count_window)) +
  geom_line() +
  ggtitle("Total Pollen Counts of Poleno4 for Hourly Intervals") +
  labs(x = "Date", y = "Pollen Count per Hourly Interval")
...
ts_autocorr_tb %>% 
  mutate(lag01 = lag(count_window, 1),
         lag02 = lag(count_window, 2),
         lag04 = lag(count_window, 4),
         lag06 = lag(count_window, 6),
         lag12 = lag(count_window, 12),
         lag24 = lag(count_window, 24),
         lag48 = lag(count_window, 48),
         lag72 = lag(count_window, 72),
         lag96 = lag(count_window, 96)) %>% 
  pivot_longer(cols = lag01:lag96, names_to = "lag", values_to = "value") %>%
  ggplot(aes(x = count_window, y = value)) +
...
dcmp <- ts_autocorr %>%
  model(STL(count_window ~ season(window = 24)))
dcmp_components <- components(dcmp)
dcmp_components %>% autoplot()
...
gg_acf <- ggplot(data = poleno_acf_df_remainder, mapping = aes(x = lag, y = acf)) +
  geom_hline(aes(yintercept = 0)) +
  geom_segment(mapping = aes(xend = lag, yend = 0)) +
  geom_hline(aes(yintercept = ciline), linetype = 2) +
  geom_hline(aes(yintercept = -ciline), linetype = 2)
...
gg_pacf <- ggplot(data = poleno_pacf_df_remainder, mapping = aes(x = lag, y = acf)) +
  geom_hline(aes(yintercept = 0)) +
  geom_segment(mapping = aes(xend = lag, yend = 0)) +
  geom_hline(aes(yintercept = ciline), linetype = 2) +
  geom_hline(aes(yintercept = -ciline), linetype = 2)

ggarrange(gg_acf, gg_pacf, nrow = 2)

forecast::auto.arima(dcmp_components$remainder)
forecast::auto.arima(ts_autocorr)
...
```{r}
ts_autocorr_tb_daily <- data_daily_comb %>% 
  filter(trap == "poleno4") %>% 
  select(timestamp, total) %>% 
  mutate(total = if_else(is.na(total), 0, total),
         total = log(total + 1)) %>% 
  rename(end_window = timestamp, count_window = total)

ts_autocorr_daily <- as_tsibble(ts_autocorr_tb_daily, regular = TRUE, index = end_window)

dcmp <- ts_autocorr_daily %>%
  model(STL(count_window ~ season()))
dcmp_components <- components(dcmp)
dcmp_components %>% autoplot()
