...
occurence_raw %>% 
  ...
  filter(name %in% c(species_selection, "Total")) %>% 
...
data_conc_diff <- data_errors_conc_raw %>% 
  filter(type == "Poaceae") %>% # Feel free to filter by species
...
data_conc_diff %>% 
  ...
  labs(x = "", y = "abs(trapᵢ - trapⱼ) / mean(trapᵢ, trapⱼ)", title = "Relative Pairwise Differences for Daily Average Concentrations") +
  scale_y_continuous(labels = scales::percent)

# data_conc_plot <- data_conc_diff %>% 
#   group_by(group) %>% # Select group or type here
#   summarise(meandiff24 = mean(diff24, na.rm = TRUE),
#             meandiff28 = mean(diff28, na.rm = TRUE),
#             meandiff48 = mean(diff48, na.rm = TRUE),
#             sddiff24 = sd(diff24, na.rm = TRUE),
#             sddiff28 = sd(diff28, na.rm = TRUE),
#             sddiff48 = sd(diff48, na.rm = TRUE)) %>% 
#   mutate(group = factor(group, levels = c("Group00_10", "Group10_20", "Group20_50", "Group50_100", "Group100_300", "Group300"))) %>% 
#   ungroup %>% 
#   arrange(group)
# 
# data_conc_plot %>% 
#   pivot_longer(meandiff24:meandiff48, names_to = "traps") %>% 
#   mutate(traps = rep(c("24", "28", "48"), 6)) %>% 
#   select(group, traps, mean = value) %>% 
#   bind_cols(
#     data_conc_plot %>% 
#       pivot_longer(sddiff24:sddiff48, names_to = "names", values_to = "sd") %>% 
#       select(sd)
#   ) %>%
#   mutate(lower = if_else(mean - 1.96 * sd >= 0, mean - 1.96 * sd, NA_real_),
#          upper = mean + 1.96 * sd) %>% 
#   select(-sd) %>% 
#   pivot_longer(mean:upper, names_to = "type", values_to = "values") %>% 
#   mutate(type = factor(type, levels = c("upper", "mean", "lower"), labels = c("+ 1.96 SD", "Mean", "- 1.96 SD"))) %>% 
#   ggplot(aes(x = group, y = values, col = traps, fill = traps, shape = type)) +
#   geom_point() +
#   scale_shape_manual(values = c(24, 3, 25)) +
#   labs(x = "", y = "Mean Differences", title = "Relative Pairwise Differences from Common Mean")
...
# There is an unsolved error in the nparcomp source-code: https://stackoverflow.com/questions/24047659/dataset-limitation-in-r-package-nparcomp
...
# The Sysadmin does not allow to save kabel as png (need phantomjs), hence the tables were saved manually.
...
ggsave("figure9_pairwise_contrasts.png", gg_contrasts, width = 10, height = 7, dpi = 300)
