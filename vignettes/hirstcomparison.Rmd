...
data_raw <- map(data_raw, ~.x %>% filter(datetime > as_datetime("2013-04-01 23:00:00"),
                                         datetime <= as_datetime("2013-06-29 23:00:00"))) 
```
...
data_hours3_raw <- map(data_raw, ~.x %>% 
  mutate(hour = case_when(
    ...
    hour >= 15 & hour < 18 ~ 18,
    hour >= 18 & hour < 21 ~ 21,
    hour >= 21 & hour < 24 ~ 0 # For the next day already
    )
  ) %>% 
  group_by(date, hour, trap, line) %>% 
  summarise_at(vars(all_of(species_selection), Total, Group1, Group2, Group3, Group4, Group5), ~mean(., na.rm = TRUE)) %>% 
      ...
      for (i in 1:1000){
        ...
        if (j == "Poaceae"){
          season_start[[i]] <- tibble(date = as.Date(as.Date("2013-04-03"):as.Date("2013-06-30"), origin = "1970-01-01"), !!j := sampled_days_random) %>% 
            AeRobiology::calculate_ps(method = "clinical", plot = FALSE, th.pollen = 3, th.sum = 30) %>% 
            pull(st.dt)
        } else {
          season_start[[i]] <- tibble(date = as.Date(as.Date("2013-04-03"):as.Date("2013-06-30"), origin = "1970-01-01"), !!j := sampled_days_random) %>% 
...
ggarrange(plotlist = gg_season_start[-6], ncol = 2, nrow = 3) %>% 
  annotate_figure(top = "Histrograms of Sampled Starts of Bloomin Season for Top-5 Species",
                  bottom = text_grob("The start was calculated for 1000 random samples for the top 5 species. The samples were generated with the t-distribution fitted to the relative differences of one trap from the mean measurements.", color = swatch()[1], face = "italic", size = 10))
...
data_sampling_conc <- data_sampling %>% 
  select(-type) %>% 
  pivot_longer(Castanea:Picea, names_to = "type", values_to = "conc") %>% 
  mutate(Group10_20 = if_else(between(conc, 10, 20), conc, NA_real_),
         Group20_50 = if_else(between(conc, 20, 50), conc, NA_real_),
         Group50_100 = if_else(between(conc, 50, 100), conc, NA_real_),
         Group100_500 = if_else(between(conc, 100, 500), conc, NA_real_),
         Group500 = if_else(conc > 500, conc, NA_real_)) 

gg_conc_dens <- list()
gg_conc_hist <- list()

labels_y <- list(0.14, 0.04, 0.02, 0.003, 0.0015)
labels_y_hist <- list(7.2, 5, 4, 5, 2.5)
...
for (j in c("Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")){
  obs <- data_sampling_conc %>% 
    filter(!is.na(!!sym(j))) %>% 
    group_by(trap) %>% 
    summarise(n()) %>% 
    pull 
  obs <- paste0(paste0("trap", c(2, 4, 8)), ": ", obs, collapse = "\n")
  obs <- paste("# of Observations \n", obs)
  
  gg_conc_dens[[j]] <- data_sampling_conc %>% 
    filter(!is.na(!!sym(j)))  %>% 
    ggplot() +
    ...
    geom_label(label = obs, aes(x = max(!!sym(j)) * 0.9), y = labels_y[[j]])
}
...
for (j in c("Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")){
  ...
  gg_conc_hist[[j]] <- data_sampling_conc %>% 
    filter(!is.na(!!sym(j)))  %>% 
    ggplot() +
    geom_histogram(data = subset(data_sampling_conc, trap == 4), aes(x = !!sym(j)), col = swatch()[3], fill = swatch()[3], alpha = 0.06, bins = 20) + 
    geom_histogram(data = subset(data_sampling_conc, trap == 8), aes(x = !!sym(j)), col = swatch()[4], fill = swatch()[4], alpha = 0.06, bins = 20) + 
    geom_histogram(data = subset(data_sampling_conc, trap == 2), aes(x = !!sym(j)), col = swatch()[2], fill = swatch()[2], alpha = 0.06, bins = 20) + 
    geom_label(label = obs, aes(x = max(!!sym(j)) * 0.9), y = labels_y_hist[[j]])
}
...
for (j in c("Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")){
  samples_conc[[j]] <- plot_hist_dt(data_sampling_conc, j, n_test = 10, samples = 1000)
  gg_hist_conc[[j]] <- plot_hist_dt(data_sampling_conc, j, n_test = 10, samples = 1000, plots = TRUE)
}
