...
norm_errors_abs <- with(errors, tibble(x = x_norm, y = dnorm(x_norm, mean = mean(error), sd = sd(error)/2.5)))
norm_errors_rel <- with(errors, tibble(x = x_norm, y = dnorm(x_norm, mean = mean(error_rel), sd = sd(error_rel))))
...
# x_t <- seq(-2, 2, length.out = 100)
# t_shape <- fitdistr(rel_errors$error, "t")
# t_errors <- tibble(x = x_t, y = dt(x_t, df = t_shape$estimate[3]))
# gg_t_rel <- errors %>% 
#   ggplot(aes(x=error , y = ..density..)) +
