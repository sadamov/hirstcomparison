...
# map2(data, names(data), ~Amelia::missmap(as.data.frame(.x), y.labels = "", y.at = 1, col = ggthemr::swatch()[c(4,2)], main = paste("Missingnes Map for", .y)))
data[[1]] %>% as.data.frame() %>% Amelia::missmap(y.labels = "", y.at = 1, col = ggthemr::swatch()[c(4,2)], main = "Missingness Map for Trap 2 - Line 2")

```

For now we leave these measurements as NA and will not plot them or impute values for statistical analyses.
...
# We could exclude all observations where the measurement of all lines and traps is lower than 10 Pollen/m^3, as discussed with Regula Gehrig. But this can be achieved ba looking at the different concentration groups. To be discussed whether the total should not contain <10 Pollen/m³ concentrations.
data <- data %>% 
  bind_rows %>%   
  group_by(datetime) %>% 
...
data[[1]] %>% as.data.frame() %>% Amelia::missmap(y.labels = "", y.at = 1, col = ggthemr::swatch()[c(4,2)], main = "Missingness Map for Trap 2 - Line 2 after data Preparation")
...
spi <- data_daily %>% # Seasonal Pollen Integral
  bind_rows %>% 
  group_by(date, trap) %>% 
  summarise_at(vars(all_of(species_selection), Total, Group10_20, Group20_50, Group50_100, Group100_300, Group300), ~mean(., na.rm = TRUE)) %>% 
  ungroup() %>% 
...
gg_ab1 <- data_altman %>% 
  ...
  coord_cartesian(xlim = c(3, 8), ylim = c(-2, 2)) +  
  geom_abline(slope = 0, intercept = 0, col = ggthemr::swatch()[4], alpha = 0.8) +
  geom_abline(slope = 0, intercept = sd_diff[1] * 2, col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_abline(slope = 0, intercept = sd_diff[1] * (-2), col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_smooth(alpha = 0.2) +
  labs(x = "Difference(trap4 - trap2)", y = "Mean(test4, test2)")
...
gg_ab2 <- data_altman %>% 
  ...
  coord_cartesian(xlim = c(3, 8), ylim = c(-2, 2)) +  
  geom_abline(slope = 0, intercept = 0, col = ggthemr::swatch()[4], alpha = 0.8) +
  geom_abline(slope = 0, intercept = sd_diff[2] * 2, col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_abline(slope = 0, intercept = sd_diff[2] * (-2), col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_smooth(alpha = 0.2) +
  labs(x = "Difference(trap8 - trap2)", y = "Mean(test8, test2)")
...
gg_ab3 <- data_altman %>% 
  ...
  coord_cartesian(xlim = c(3, 8), ylim = c(-2, 2)) +  
  geom_abline(slope = 0, intercept = 0, col = ggthemr::swatch()[4], alpha = 0.8) +
  geom_abline(slope = 0, intercept = sd_diff[3] * 2, col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_abline(slope = 0, intercept = sd_diff[3] * (-2), col = ggthemr::swatch()[4], alpha = 0.8, linetype = 3) +
  geom_smooth(alpha = 0.2) +
  labs(x = "Difference(trap8 - trap4)", y = "Mean(test8, test4)")

title <- "Altman-Bland Plots for all Traps"

ggarrange(gg_ab1, gg_ab2, gg_ab3, nrow = 3) %>%
  annotate_figure( top = title, bottom = text_grob("Pairwise comparison between traps; grey line shows the Loess smother; the red line shows a theroratical perfect agreement between two traps of zero. \n The dashed red line shows the 2 * sd of the differences, where we expect the points to lie within.", color = ggthemr::swatch()[1], face = "italic", size = 12))
...
data_transformed %>% 
  ...
  # geom_freqpoly(col = ggthemr::swatch()[3], bins = 30) +
  # geom_density(col = ggthemr::swatch()[5]) +
  geom_vline(xintercept = median(data_transformed %>% pull(!!sym(species)), na.rm = TRUE), col = ggthemr::swatch()[4]) +
