...
conc_y_label <- c(2, 1, 0.8, 1, 1.1, 1.2) # This has to be adjusted depending on the temporal averaging window. You could probably find a smart rule, I just did not find it :-)
names(conc_y_label) <- c("Group00_10", "Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")

for (j in c("Group00_10", "Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")){
  gg_hist_conc[[j]] <- plot_hist_dt(data_errors_conc, j, n_test = 1, samples = 1, plots = TRUE, tdist = FALSE, y_label = conc_y_label[j])
}

data_errors_conc %>% select(Group10_20) %>% unique
...
data_contr_conc <- map(data_contr_tf, ~.x %>% 
  ...
  mutate(conc = case_when(
    ...
    mean >= log(100) & mean < log(500) ~ "Group100_500",
    mean >= log(500) ~ "Group500"
  )) %>% 
  select(timestamp, trap, value, conc, type) %>% 
  pivot_wider(names_from = conc, values_from = value))


species_contr <- c("Group0_10", "Group10_20", "Group20_50", "Group50_100", "Group100_500", "Group500")
