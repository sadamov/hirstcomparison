...
>>>>>>> 77a40a232a6736859e3f940432b6a570bc87af77
```
...
sd_tb_daily <- map(sd_list_daily, ~.x %>% 
  ...
  mutate(sd = sd(value),
         ...
         cv = sd/mean) %>% 
  filter(trap == "trap2",
         name %in% c("Fraxinus", "Poaceae", "Betula", "Quercus", "Pinus")) %>% 
...
## Deviations of the Individual Measurements
...
sd_2hour_ind <- data_hours2_raw %>% 
  ...
  summarise_at(vars(all_of(species_selection)), ~mean(., na.rm = TRUE)) %>% 
  ungroup() %>% 
  pivot_longer(Castanea:Picea, names_to = "name", values_to = "value") %>% 
  group_by(name) %>% 
  mutate(sd_ind = sd(value),
         mean_ind = mean(value),
         se_ind = sd_ind / n(),
         cv_ind = sd_ind/mean_ind) %>% 
    filter(name %in% c("Fraxinus", "Poaceae", "Betula", "Quercus", "Pinus"),
         trap == 2,
...
sd_mean_temp <- sd_temp %>% 
  bind_cols(mean_temp) %>% 
  mutate(Total = Total / Total1,
         Fraxinus = Fraxinus / Fraxinus1,
         Poaceae = Poaceae / Poaceae1,
         Betula = Betula / Betula1,
         Quercus = Quercus / Quercus1,
         Pinus = Pinus / Pinus1
         ) %>% 
