...
data_hours3 <- map(data, ~.x %>% 
  mutate(hour = case_when(
    ...
    hour >= 3 & hour < 6 ~ "06:00",
    hour >= 6 & hour < 9 ~ "09:00",
    hour >= 9 & hour < 12 ~ "12:00",
    hour >= 12 & hour < 15 ~ "15:00",
    hour >= 15 & hour < 18 ~ "18:00",
    hour >= 18 & hour < 21 ~ "21:00",
...
number_traps <- map2(number_of_measurements, timesteps, ~ bind_rows(.x %>% 
  ...
  mutate_at(vars(all_of(species_selection), Total), ~ scales::percent(. / !!.y)) %>% 
...
plot <- plot_hirst(species = species, resolution = resolution, group = "trap", traps = c(2, 4, 8), rm_zeros = TRUE)
plot
...
for (j in c("Group1", "Group2", "Group3", "Group4", "Group5", "Total")){
  ...
  t_shape[[j]] <- try(QRM::fit.st(errors$error[!is.na(errors$error)])) # Actually lead to the same result as the bootstrapped version below
  ...
  if (class(t_shape[[j]]) != "try-error"){
    m <- t_shape[[j]]$par.ests[2]
    s <- t_shape[[j]]$par.ests[3]
    df <- t_shape[[j]]$par.ests[1]
    t_errors <- tibble(x = x, y = dt((x - m) / s, df = df) / s)
    ...
    samples[[j]] <- rt(1000, df = df) * s + m # This gives us 10k random samples for the absolute log-differences from the mean. See here again for supplying location and scale parameteres to t-distribution in Base-R: https://en.wikipedia.org/wiki/Location%E2%80%93scale_family
  ...
  if (class(t_shape) != "try-error"){
    ...
    geom_label(label = paste("SD:", round(sd, 3), "\n DF:", round(df, 2), "\n #:", nrow(errors %>% filter(!is.na(error)))), aes(x = 2, y = 1), size = 3)
  }
    
}


ggarrange(plotlist = gg, nrow = 3, ncol = 2) %>% 
...
for (j in c("Group1", "Group2", "Group3", "Group4", "Group5", "Total")){
  ...
  relative_differences <- exp(samples[[j]]) # Samples were generated in step above, make sure that same species were applied
  ...
  for (i in 1:1000){
    sampled_days_random[[i]] <- map(sampled_days, ~sample(.x, 1, replace = FALSE)) %>% flatten_dbl() %>% sum
  }
  
  sampled_spi[[j]] <- sampled_days_random %>% flatten_dbl %>% as_tibble()
  
  gg_spi[[j]] <- sampled_spi[[j]] %>% 
    ggplot(aes(x = value)) +
    geom_boxplot() +
    coord_cartesian(xlim = c(0, 5e4)) + # quantile(sampled_spi[[j]]$value, probs = c(0.2, 0.8)) %>% as.numeric
    labs(x = j)
}

ggarrange(plotlist = gg_spi[c(-2, -5)]) %>% 
  annotate_figure(top = "Boxplot of Sampled SPI for Various Species",
                  bottom = text_grob("We are looking at absolute differences from the mean fitted with a density kernel estimator (red) and a Student-t distribution (black). \n In the left box we see the bootstrapped P-value for the two goodness-of-fit tests. In the right box we see the sd and degrees of freedom of the fitted t-Distribution.", color = swatch()[1], face = "italic", size = 10))
...
for (j in c("Fraxinus", "Poaceae", "Betula", "Quercus", "Pinus", "Total")){
  errors <- data_transformed %>% 
    select(!!sym(j), trap, timestamp) %>% 
    pivot_wider(names_from = trap, values_from = !!sym(j), timestamp) %>% 
    setNames(c("timestamp", paste0("trap", c(2, 4, 8)))) %>% 
  ...
  samples[[j]] <- rt(10000, df = df) * s + m # This gives us 10k random samples for the absolute log-differences from the mean. See here again for supplying location and scale parameteres to t-distribution in Base-R: https://en.wikipedia.org/wiki/Location%E2%80%93scale_family
...
for (j in c("Fraxinus", "Poaceae", "Betula", "Quercus", "Pinus", "Total")){
  ...
  gg_spi[[j]] <- sampled_spi[[j]] %>% 
    ggplot(aes(x = value)) +
    geom_boxplot() +
    coord_cartesian(xlim = c(0, 8e4)) +
    labs(x = j)
}

map(samples, ~.x %>%  summary)

ggarrange(plotlist = gg_spi[], nrow = 3, ncol = 2) %>% 
  annotate_figure(top = "Boxplot of Sampled SPI for Various Species",
                  bottom = text_grob("We are looking at absolute differences from the mean fitted with a density kernel estimator (red) and a Student-t distribution (black). \n In the left box we see the bootstrapped P-value for the two goodness-of-fit tests. In the right box we see the sd and degrees of freedom of the fitted t-Distribution.", color = swatch()[1], face = "italic", size = 10))
