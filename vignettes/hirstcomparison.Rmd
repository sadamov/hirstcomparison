...
```{r}
rel_errors <- data_corr %>% 
  mutate(mean = (trap2 + trap4 + trap8) / 3) %>% 
  mutate_at(vars(paste0("trap", c(2, 4, 8))), ~(. - mean)/mean) %>% 
  pivot_longer(cols = trap2:trap8, values_to = "error", names_to = "trap") %>% 
  select(-mean)

x <- seq(-1, 1, length.out = 100)
norm_errors <- with(rel_errors, tibble(x = x, y = dnorm(x, mean = mean(error), sd = sd(error)/5)))
t_shape <- fitdistr(rel_errors$error, "t")
t_errors <- tibble(x = x, y = dt(x, df = 15))
...
rel_errors %>% 
  ...
  geom_line(data = norm_errors, aes(x = x, y = y), col = ggthemr::swatch()[6]) +
  geom_line(data = t_errors, aes(x = x, y = y), col = ggthemr::swatch()[8])
