...
```{r eval = FALSE}
gg_missmap <- data[[1]] %>%  as.data.frame() %>% Amelia::missmap(y.labels = "", y.at = 1, col = swatch()[c(4,2)], main = "Missingness Map for Trap 2 - Line 2 after data Preparation")
...
data_aggs <- list(data_hourly, data_hours2, data_hours3, data_hours6, data_hours12, data_daily)
...
kable_number_traps <- number_traps %>% 
  bind_rows() %>% 
  kable() %>% 
  kable_styling("striped") %>% 
  pack_rows("Hourly Averages", 1, 4) %>% 
  pack_rows("2-Hour Averages", 5, 8) %>% 
  pack_rows("3-Hour Averages", 9, 12) %>% 
  pack_rows("6-Hour Averages", 13, 16) %>% 
  pack_rows("12-Hour Averages", 17, 20) %>% 
  pack_rows("Daily Averages", 21, 24) %>% 
...
```{r}
save_kable(kable_number_traps, "table_number_traps.png")
```
